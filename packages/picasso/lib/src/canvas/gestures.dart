...
mixin CanvasGestureMixin on PicassoCanvasState {
  ...
  double snapRotation(double angle, PicassoLayer layer,
    ...
    if (!widget.settings.snapRotation || !layer.hasFlag(LayerFlags.rotatable)) return angle;
  ...
  TransformData trySnapPosition(PicassoLayer layer, TransformData transform,
      {double tolerance = 16}) {
    if (!widget.settings.snapPosition || !layer.hasFlag(LayerFlags.snapping)) return transform;
  ...
  void handleScaleStart(ScaleStartDetails details,
    ...
    for (var i = layerCount - 1; i >= 0; i--) {
      var layer = layers[i];
      if (layer.hasFlag(LayerFlags.passthrough) || layer.locked || layer.hidden) continue;
  ...
  void _performScaleStart(ScaleStartDetails details,
    ...
    if (layer.hasFlag(LayerFlags.promoting) && widget.settings.layerPromotion) {
      removeLayer(layer);
  ...
  void handleScaleUpdate(ScaleUpdateDetails details,
    ...
    if (selectedLayer != null) {
      var transform = transforms[selectedLayer!];
      var layer = layers[selectedLayer!];
      var isMovable = layer.hasFlag(LayerFlags.movable);
      var isScalable = layer.hasFlag(LayerFlags.scalable);
      var movedTransform = transform.copyWith(
          scale: isScalable ? _scaleValue * details.scale : _scaleValue,
          rotation: layer.hasFlag(LayerFlags.rotatable)
          ...
          y: isMovable
              ...
              : _yValue);
      if (isMovable) movedTransform = trySnapPosition(layer, movedTransform);
      var sizeAfter =
      ...
      var adjustedTransform = !isScalable ? movedTransform
          : movedTransform.copyWith(
              x: movedTransform.x - dx, y: movedTransform.y - dy);

      if (layer.hasFlag(LayerFlags.cover) && isMovable) {
      ...
      if (layer.hasFlag(LayerFlags.deletable)) {
        if (isSingleMove) {
          showDeleteIcon = true;
        } else {
